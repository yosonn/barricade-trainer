...
top, while a computer-first game keeps blue on top. Touch wall previews are
offset above the finger, and the phone command layout keeps the mode bar, both
player cards, decision status, full board, wall tools, live status, and essential
controls inside one viewport. Desktop layout and AI battle features are retained.

Version `2026.07.12.01` keeps the trainer board visible even when analysis is
slow or unavailable, reduces fast state-sync work after each move, and replaces
the large wall cards with compact draggable wall pieces and board previews.
Non-Expert requests now tolerate deployment cold starts without changing their
configured search budget. Version `2026.07.11.04` enlarged the board while
...
from the remaining height while the right controls retain their own scroll.
The trainer page has been rebuilt with the same command-center design,
responsive behavior, model/latency/sync indicators, and compact settings as the
AI battle page. Trainer history blur no longer launches a hidden analysis.

Version `2026.07.11.02` fixes the remaining intermittent Expert visual delay.
Optimistic rendering now accepts every well-formed pawn or wall action instead
of depending on the locally ranked `legal_actions` subset, so Expert walls that
fall outside the local top wall candidates still paint immediately. Hidden
history-field blur analysis was removed, and the accidental `[object
FocusEvent]` status message is fixed. The AI battle page now uses a responsive
command-center layout with a complete board, prominent recommendation/model/
sync status, measured response time, and compact secondary settings.
...
needs to copy onto the real/visual board after the computer move has already
been applied locally, while still avoiding new Expert thinking for human turns.
